...
\begin{tikzpicture}[scale=2. 5]
  ...
  \node[above right] at (-1,0. 05) {\footnotesize 14 zeros};
  
  % Poles (14 poles distributed inside unit circle)
  \foreach \i in {1,...,14} {
    \pgfmathsetmacro{\angle}{180 - 180*(\i-0.5)/14}
    \pgfmathsetmacro{\radius}{0.85}
    \node[mark=x, mark size=3pt, color=red] at (\angle:\radius) {};
  }
  
  \node[below] at (0,-1.5) {\textbf{All zeros at $z=-1$, poles inside unit circle}};
\end{tikzpicture}
\end{center}

\textbf{Key Features:}
\begin{itemize}
  \item 14 zeros clustered at $z = -1$ create strong attenuation at $\omega = \pi$
  \item Poles uniformly distributed inside unit circle
  \item Stable (all poles inside unit circle)
\end{itemize}
...
\begin{tikzpicture}[scale=2.5]
  ...
  % 8 zeros at z = -1
  \node[circle, fill=white, draw=black, thick, minimum size=8pt, inner sep=0] at (-1,0) {};
  \node[above right] at (-1,0.05) {\footnotesize 8 zeros};
  
  % Poles (8 poles on ellipse inside unit circle)
  \foreach \i in {1,...,8} {
  ...
  \node[below] at (0,-1.5) {\textbf{All zeros at $z=-1$, poles on ellipse}};
\end{tikzpicture}
\end{center}

\textbf{Key Features:}
\begin{itemize}
  \item 8 zeros at $z = -1$ (like Butterworth, enables efficient implementation)
  \item Poles on ellipse inside unit circle (tighter than Butterworth)
  \item Equiripple passband from pole placement
\end{itemize}
