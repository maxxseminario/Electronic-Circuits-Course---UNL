...
\begin{frame}{Why Study Op-Amp Specifications?}
    ...
    \column{0.48\textwidth}
        ...
        \begin{itemize}
            \item \textbf{Ideal}:  Simple analysis, perfect behavior
            \item \textbf{Real}:  Practical limitations exist
            \item Ignoring specifications results in circuit failure 
        \end{itemize}
...
\begin{frame}{Closed-Loop Frequency Response}
    ...
    \column{0.48\textwidth}
        ...
        \begin{circuitikz}[scale=0.85]
            % Op-amp
            \draw (0,0) node[op amp, yscale=0.7] (opamp) {};
            ...
            \draw (out) -- ++(0,1.5) to[R, l=$R_f$] ++(-5.0,0) coordinate (fb);
            \draw (fb) to[R, l_=$R_i$] (fb |- 0,-2) node[ground] {};
            \draw (fb) -- (fb |- opamp.-) -- (opamp.-);
        \end{circuitikz}
    ...
    \column{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \node[blue, right] at (axis cs:10,5) {$G = 1$ (0 dB)};
            
            % G = 10
            \addplot[red, very thick, samples=200] {20 - 20*log10(1 + x/100000)};
            \node[red, right] at (axis cs:10,25) {$G = 10$ (20 dB)};
            
            % G = 100
            \addplot[green!60!black, very thick, samples=200] {40 - 20*log10(1 + x/10000)};
            \node[green!60!black, right] at (axis cs:10,45) {$G = 100$ (40 dB)};
            
            % Open-loop
            \addplot[black, dashed, samples=200] {106 - 20*log10(1 + x/10)};
            \node[black, right] at (axis cs:100,86) {Open-loop};
...
\begin{frame}{Phase Margin and Stability}
    ...
    \draw[dashed, red] (axis cs:100000,-180) -- (axis cs:100000,0);
    \node[red] at (axis cs:200000,-45) {$f_t$};

    % Phase margin annotation
    \draw[<->, green!60!black, very thick] (axis cs:200000,-180) -- (axis cs:200000,-100);
    \node[green!60!black] at (axis cs:350000,-150) {PM $\approx 90^\circ$};
...
\begin{frame}{Small Signal AC Model}
    ...
    \column{0.48\textwidth}
        ...
        \begin{circuitikz}[scale=0.9]
            ...
            % VCVS
            \draw (3,0) node[diamond, draw, thick, minimum size=1.2cm] (vcvs) {$A(f) v_d$};
...
\begin{frame}{Closed-Loop Gain Derivation}
    ...
    \vspace{-0.6cm}
        ...
        \begin{circuitikz}[scale=0.85]
            ...
            % Feedback network
            \draw (opamp.out) to[short, *-] ++(0.5,0) coordinate (out) to[short, -o] ++(0.5,0) node[right] {$v_{out}$};
            \draw (out) -- ++(0,1.5) to[R, l_=$R_f$] ++(-5.0,0) coordinate (fb);
            \draw (fb) to[R, l_=$R_i$] (fb |- 0,-2) node[ground] {};
            \draw (fb) -- (fb |- opamp.-) -- (opamp.-);
        \end{circuitikz}
        \caption{Non-inverting amplifier}
...
\begin{frame}{Slew Rate:   Large Signal Limitation}
    ...
    \column{0.48\textwidth}
        ...
        \begin{tikzpicture}
            ...
            \node[green! 60!black] at (axis cs:8,0.5) {$0.5$ V/$\mu$s};
...
\begin{frame}{Full-Power Bandwidth vs.\ Small-Signal Bandwidth}
    ...
    \node[blue, right] at (axis cs:10,12) {Small-signal limit};

    \addplot[red, very thick, samples=200, domain=10:1000000]
  {min(15, 0.5e6/(2*pi*x))};
    \node[red, above left] at (axis cs:5000,1.5) {Slew-rate limit};

    \draw[dashed, green!60!black] (axis cs:7960,0) -- (axis cs:7960,10);
    \node[green!60!black, rotate=90] at (axis cs:20000,5) {$f_{FP} = 8$ kHz};

    \end{axis}
    \end{tikzpicture}
    \caption{$G=1$, SR$=0.5$ V/$\mu$s, $f_t=1$ MHz}
    \end{figure}
...
\begin{frame}{Power Supply Rejection Ratio (PSRR)}
    ...
    \column{0.48\textwidth}
        ...
        \begin{block}{ For low-noise applications}
            \begin{itemize}
                \item Use well-regulated supplies
                \item Add bypass capacitors 
...
\begin{frame}{Input and Output Impedances (Real)}
    ...
    \column{0.48\textwidth}
        ...
        \begin{block}{ Minimum load resistance (Power Limited)}
...
\begin{frame}{Summary:  Real Op-Amp Specifications}
    ...
    \column{0.48\textwidth}
        ...
        \begin{itemize}
            \item Open-loop gain: $A_0$ (finite)
...
\begin{frame}{Key Formulas Reference}
    ...
    \hline
    Gain-bandwidth product & $G \times BW = f_t$ \\
    \hline
    \end{tabular}
    \end{table}
    ...
    \hline
    Offset error & $V_{out,error} = V_{OS} \cdot G$ \\
    \hline
    \end{tabular}
    \end{table}
    \end{columns}
...
\begin{frame}{Practice Problem 3 Solution}
    ...
    \textbf{Therefore option A is suitable }
    ...
    \textbf{Therefore option B is also suitable (but overkill)}
