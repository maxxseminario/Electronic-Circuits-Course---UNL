...
\begin{frame}{Review:  The Ideal Op-Amp}
    ...
    \column{0.48\textwidth}
        ...
        \begin{circuitikz}[scale=0.7]
            % Op-amp
            \node[op amp, yscale=1.5] (opamp) at (0,0) {};

            % Labels
            \node at (opamp.-)  [left, xshift=-0.1cm] {$v_-$};
            \node at (opamp.+)  [left, xshift=-0.1cm] {$v_+$};
            \node at (opamp.out)[right, xshift=0.3cm] {$v_{out}$};

            % Current annotations (horizontal, shifted left so they don't overlap labels)
            \draw[->, red, thick]
                ($(opamp.-)+(-2.0,0)$) -- ($(opamp.-)+(-1.1,0)$)
                node[left, black] at ($(opamp.-)+(-2.0,0)$) {$i_-=0$};
...
\begin{frame}{The Noninverting Amplifier}
    ...
    \column{0.48\textwidth}
        ...
        \begin{circuitikz}[scale=1.0, every node/.style={font=\small}]
            ...
            % Define vin 
            \coordinate (vin) at ($(opamp.+)+(-1,0)$);

            % Vin node 
            \draw (vin) node[left] {$v_{in}$} to[short, o-] (opamp.+);

            % Output
            \draw (opamp.out) to[short, -o] (5.5,0) node[right] {$v_{out}$};

            % Feedback resistor Rf 
            \draw (opamp.out) -- ++(0,1.3)
                to[R, l^=$R_f$] ++(-2.0,0) coordinate (rfL)  
                -- ++(-1.0,0) coordinate (rfLext)            
                |- (opamp.-);                              

            \coordinate (invleft) at ($(opamp.-)+(-2.0,0)$);
            \draw (opamp.-) -- (invleft);

            % R1 from (-) input to ground 
            \draw (invleft) to[R, l_=$R_1$] ++(0,-2.0) node[ground] {};
...
\begin{frame}{Noninverting Amplifier: Key Results}
    ...
    \column{0.48\textwidth}
        ...
        \begin{itemize}
            \item[\mehitem] Always positive (no inversion)
            \item Minimum gain is 1
            \item[\gooditem] Set by resistor ratio
        \end{itemize}
        ...
        \begin{itemize}
            \item[\gooditem] Infinite (ideally)
            \item[\gooditem] No loading of source
        \end{itemize}
    ...
    \column{0.48\textwidth}
        ...
        \textbf{Example 3}:  Gain of 1
        \begin{itemize}
            \item $R_1 = \infty$, $R_f = 0$
            \item $A_v = 1 + 0 = 1$
        \end{itemize}
...
\begin{frame}{Noninverting Amplifier: Voltage Follower (Unity-Gain Buffer)}
    ...
    \column{0.48\textwidth}
        ...
        \textbf{Key Features}:
        \begin{itemize}
            \item[\gooditem] Output tracks the input: $v_{out} \approx v_{in}$
            \item[\gooditem] High input impedance, allows it to sense the input without loading it
            \item[\gooditem] Low output impedance allows it to drive heavy loads
            \item[\gooditem] Used to isolate stages, prevent loading effects
        \end{itemize}
...
\begin{frame}{The Summing Amplifier}
    ...
    \column{0.6\textwidth}
        ...
        \begin{circuitikz}[scale=0.9, every node/.style={font=\small}]
            ...
            % Tie the three inputs into the summing node with a vertical bus + junction dot
            \draw ([yshift=1.2cm]sum) -- ([yshift=-1.2cm]sum);
            \fill (sum) circle (1.2pt);

            % Connect summing node to inverting input
            \draw (sum) -- (opamp.-);

            % Non-inverting input to ground
            \draw (opamp.+) -- ++(0,-1.2) node[ground] {};
...
\begin{frame}{Summing Amplifier: Applications}
    ...
    \column{0.6\textwidth}
        ...
        \centering
        \begin{circuitikz}[scale=0.9, every node/.style={font=\small}]
            % Op-amp (centered, like the first slide)
            \node[op amp, scale=1.0] (opamp) at (3.2,0) {};

            % Summing node coordinate (aligned with opamp - input)
            \coordinate (sum) at (1.5,0 |- opamp.-);

            % Inputs + resistors (binary-weighted)
            \draw ([yshift=1.2cm]-1.5, 0 |- opamp.-) node[left] {MSB} to[R, l=$R$, o-] ([yshift=1.2cm]sum);
            \draw (-1.5, 0 |- opamp.-) node[left] {} to[R, l=$2R$, o-] (sum);
            \draw ([yshift=-1.2cm]-1.5, 0 |- opamp.-) node[left] {LSB} to[R, l=$4R$, o-] ([yshift=-1.2cm]sum);

            % Vertical bus tying inputs into the summing node + junction dot
            \draw ([yshift=1.2cm]sum) -- ([yshift=-1.2cm]sum);
            \fill (sum) circle (1.2pt);
            ...
            % Non-inverting input to ground
            \draw (opamp.+) -- ++(0,-0.2) node[ground] {};

            % Output
            \draw (opamp.out) to[short, -o] (5.7,0) node[right] {$v_{out}$};

            % Feedback resistor (Rf = R for the DAC example)
            \draw (opamp.out) -- ++(0,1.4)
                to[R, l^=$R$] ++(-2.0,0)
                -| (opamp.-);
        \end{circuitikz}
        \caption{Binary-weighted 3-bit DAC}
        \end{figure}
...
\begin{frame}{The Difference Amplifier}
    ...
    \column{0.5\textwidth}
        ...
        \centering
        \begin{circuitikz}[scale=0.8, every node/.style={font=\small}]
            % Op-amp (centered like the previous figure)
            \node[op amp, scale=1.0] (opamp) at (3.2,0) {};

            % Coordinates aligned
            \coordinate (invL) at (1.5,0 |- opamp.-); % left of V-
            \coordinate (noninvL) at (1.5,0 |- opamp.+); % left of V+

            % --- Inverting input path
            \draw (-1.5,0 |- opamp.-) node[left] {$v_1$}
                to[R, l=$R_1$, o-] (invL) -- (opamp.-);

            % Feedback resistor: opamp.out 
            \draw (opamp.out) -- ++(0,1.4)
                to[R, l^=$R_2$] ++(-2.0,0)
                -| (opamp.-);

            % --- Non-inverting input path:
            \draw (-1.5,0 |- opamp.+) node[left] {$v_2$}
                to[R, l=$R_3$, o-] (noninvL);

            % Node into V+
            \draw (noninvL) -- (opamp.+);

            % Divider to ground from the same node
            \draw (noninvL) to[R, l_=$R_4$] ++(0,-1.4) node[ground] {};
    ...
    \column{0.5\textwidth}
        ...
        Voltage at noninverting input:
        \[
        v_+ = v_- = v_2 \frac{R_4}{R_3 + R_4}
        \]
...
\begin{frame}{Difference Amplifier: Key Points}
    ...
    \column{0.48\textwidth}
        ...
        \textbf{Advantages}:
        \begin{itemize}
            \item[\gooditem] Rejects common-mode signals
            \item[\gooditem] Amplifies differential signal
            \item[\gooditem] Single op-amp solution
        \end{itemize}
        
        \textbf{Limitations}:
        \begin{itemize}
            \item[\baditem] Requires precision resistor matching
            \item[\baditem] Finite input impedance at both inputs
            \item[\mehitem] Limited CMRR (compared to instrumentation amp)
        \end{itemize}
...
\begin{frame}{The Integrator}
    ...
    \column{0.48\textwidth}
        ...
        \begin{circuitikz}[scale=0.9, every node/.style={font=\small}]
            ...
            % Put the input resistor close to the op-amp
            ...
            % Output lead shortened: move vout label left
            \draw (opamp.out) to[short, -o] (4.7,0) node[right] {$v_{out}$};

            % i_C label
            \draw[->] ($(fbTop)+(-2.55,0)$) -- ($(fbTop)+(-2.05,0)$)
    ...
    \column{0.48\textwidth}
        ...
        2. Current through $R$:
        \[
        i_R = \frac{v_{in} - 0}{R} = \frac{v_{in}}{R} = i_C
        \]
...
\begin{frame}{Integrator: Analysis}
...
\begin{frame}{The Differentiator}
    ...
    \column{0.48\textwidth}
        ...
        \centering
        \begin{circuitikz}[scale=0.9, every node/.style={font=\small}]
            % Op-amp (centered like the previous figures)
            \node[op amp, scale=1.0] (opamp) at (3.2,0) {};

            % Put the input capacitor close to the op-amp (level with V-)
            \coordinate (invL) at (2.0,0 |- opamp.-);

            % Shorter input lead: move vin to the right
            \draw (0.3,0 |- opamp.-) node[left] {$v_{in}$}
                to[C, l=$C$, o-] (invL) -- (opamp.-);

            % Non-inverting input to ground
            \draw (opamp.+) -- ++(0,-1.2) node[ground] {};

            % Feedback resistor: opamp.out -> up -> R -> back to V-
            \coordinate (fbTop) at ($(opamp.out)+(0,1.4)$);
            \draw (opamp.out) -- (fbTop)
                to[R, l^=$R$] ++(-2.0,0)
                -| (opamp.-);
...
\begin{frame}{Differentiator:  Characteristics and Issues}
    ...
    \column{0.48\textwidth}
        ...
        \begin{enumerate}
            ...
            \begin{itemize}
                \item[\baditem] High-frequency noise magnified
                \item[\baditem] Can saturate output
            \end{itemize}
            
            \item \textbf{Stability issues}
            \begin{itemize}
                \item[\baditem] Phase shift can cause oscillation
...
\begin{frame}{Summary:  Op-Amp Applications}
    ...
    \begin{block}{Standard Analysis Steps}
        \begin{enumerate}
...
\begin{frame}{Practice Problem 3 Solution}
    ...
    (c) For equal weights:  $R_1 = R_2 = R_3 = R$ 
...
\begin{frame}{Advanced Application: Instrumentation Amplifier}
    
    \begin{columns}[t]
    \column{0.48\textwidth}
        \textbf{Limitations of Simple Difference Amp}:
        \begin{itemize}
            \item[\baditem] Finite input impedance
            \item[\baditem] Limited CMRR (requires precise matching)
            \item[\mehitem] Gain-impedance tradeoff
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \item[\gooditem] Very high input impedance (both inputs)
            \item[\gooditem] Excellent CMRR (100 dB)
            \item[\gooditem] Single resistor sets gain
    ...
    \column{0.48\textwidth}
       ...
       \begin{figure}[htbp]
            \centering
            \includegraphics[width=0.9\textwidth]{Figures/inst_amp_schem.png}
            \caption{Instrumentation amplifier schematic}
            \label{fig:inst_amp_schem}
        \end{figure}
