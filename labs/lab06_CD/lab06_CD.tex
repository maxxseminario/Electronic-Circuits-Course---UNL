...
Configure your DC-offset power supply to provide the DC bias $V_G$ plus a small AC signal superimposed on it. Set the AC component to be a sine wave at 1 kHz with amplitude approximately 100 mV peak (200 mV peak-to-peak). This small amplitude ensures the transistor will show linear operation without distortion.  Gradually increase the amplitude to 500mV peak, or until you see distortion on the oscilloscope.\\
...
With the circuit from Part 2, connect various load resistors $R_L$ from the output (source) to ground. Use the following values:  1 k$\Omega$, 330$\Omega$, and 100 $\Omega$.
