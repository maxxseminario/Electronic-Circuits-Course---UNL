...
\begin{circuitikz}[american, every node/.style={font=\small}]
    ...
    \draw (0,1.5) to[short] (6,1.5) coordinate (gnd);

    % Move capacitor C further LEFT (was at x=6)
    \draw (5.2,4.5) to[C, l=$C$] (5.2,1.5);
    \draw (4.5,4.5) to[short] (6,4.5) coordinate (vraw);
    ...
    \draw (vraw) to[R, l=$R_S$, i>^=$I_S$] (8,4.5) coordinate (reg);

    % Move Zener diode DZ further LEFT (was at x=9.5)
    \draw (reg) to[short] (8.8,4.5)
          to[zD, l_=$D_Z$, v^=$V_Z$, i>_=$I_Z$] (8.8,1.5);

    % Load branch to ground
    \draw (reg) to[short] (11,4.5)
          to[R, l_=$R_L$, v^=$V_o$, i>_=$I_L$] (11,1.5);

    % Return on the same ground rail coming from the bridge
    \draw (gnd) -- (11,1.5);
