...
Begin by constructing a full-wave bridge rectifier with filter capacitor as shown in Figure \ref{fig:bridge_filtered}. This circuit should be familiar from Lab 2. Use four rectifier diodes in the bridge configuration, the 10 $\mu$F filter capacitor, and initially use a 1 k$\Omega$ load resistor. Connect the waveform generator to provide 5 V RMS at 60 Hz. The voltage limit setting of the instrument may have to be adjusted. Make sure set the output impedance setting is specified as \emph{HiZ}. Verify the polarity of the electrolytic capacitor before applying power.\\
...
Replace the load resistor with a 10 k$\Omega$ resistor and repeat the measurements. Then use the 100 k$\Omega$ resistor and repeat once more. A larger resistance draws less load current. \\
\emph{Don't disassemble the circuit as it will be used in part 3!}\\

\textbf{Report}
\begin {enumerate}
\item Present oscilloscope waveforms for at least one load condition, showing the ripple voltage clearly.
...
First, determine the characteristics of the Zener diode. (The \hyperlink{https://www.mouser.com/datasheet/3/282/1/1N5221B_1THRU1N5281B_1.pdf}{1N5225 3V zener diode} is provided in the lab) In the datasheet, find the Zener voltage $V_Z$, minimum Zener current $I_{Z,\min}$, and maximum power rating $P_{Z,\max}$. \\
...
Construct the complete regulated power supply by adding the Zener shunt regulator to the bridge rectifier from Part 1. The circuit should include the AC source, bridge rectifier, 100 $\mu$F filter capacitor, series resistor $R_S$ (the value calculated in Part 4), Zener diode, and provision for connecting different load resistors as shown in Figure \ref{fig:regulated_supply}.\\
...
Now connect a 10 k$\Omega$ load resistor and measure the output voltage. Calculate the load current as $I_L = V_o/R_L$. The output voltage should remain very close to the no-load value, demonstrating voltage regulation. Replace the load with progressively smaller resistances (between 10k$\Omega$ and 100$\Omega$), measuring the output voltage at each value. For each load condition, also measure the voltage across the series resistor to calculate $I_S$, and calculate the Zener current as $I_Z = I_S - I_L$.\\
