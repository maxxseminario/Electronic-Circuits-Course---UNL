...
The advantages of the bridge rectifier include having no requirement for a center-tapped transformer (reducing transformer cost and size) and a full utilization of the transformer secondary winding (current flows through the entire winding during both half-cycles). The disadvantages include a higher voltage drop due to two diodes in series (reducing efficiency, especially at low voltages) and the requirement of four diodes instead of two. 
...
\begin{figure}[h]
    ...
    \begin{circuitikz}[american, scale=.75]
        ...
        \draw (6,2) -- (8,2) to [R, l=$R_L$, v=$v_o$] (8,-1) to node[ground]{} (1, -1) -- (1,2) -- (2,2);
...
Simple filtered rectifier circuits, studied in Lab 2, produce a DC output voltage that varies with both input voltage changes (line variations) and load current changes (load variations). However, many electronic circuits require a stable, regulated voltage that remains constant over time. The simplest form of voltage regulation uses a Zener diode in a shunt (parallel) configuration. \\

A basic Zener shunt regulator consists of a series resistor $R_S$ (often called the dropping resistor or series resistor) connected between the unregulated DC input voltage $V_{in}$ and the load. A zener diode is connected in parallel with the load, as shown in Figure \ref{fig:shunt_reg_concept}. The zener diode is reverse-biased and operates in its breakdown region, maintaining an approximately constant voltage $V_Z$ across the load.  \\
...
The operation principle is straightforward.  The series resistor $R_S$ drops the difference between the input voltage and the desired output voltage. The Zener diode maintains a constant voltage $V_Z$ across the load by absorbing variations in current.  When the load current decreases, the Zener current increases to maintain a constant current through $R_S$ (and thus a constant voltage drop across $R_S$). When the load current increases, the Zener current decreases.  Similarly, when the input voltage increases, the Zener current increases to absorb the extra current, while maintaining constant output voltage. \\
...
\emph{Observe the behavior of a bridge rectifier with a filtered output}\\

Begin by constructing a full-wave bridge rectifier with filter capacitor as shown in Figure \ref{fig:bridge_filtered}. This circuit should be familiar from Lab 2. Use four rectifier diodes in the bridge configuration, a 47 $\mu$F filter capacitor, and initially use a 1 k$\Omega$ load resistor. Connect the waveform generator to provide 7 V RMS at 60 Hz. The voltage limit setting of the instrument may have to be adjusted. Make sure set the output impedance setting is specified as \emph{HiZ}. Verify the polarity of the electrolytic capacitor before applying power.\\
...
Using the oscilloscope, observe the output voltage $V_o$ across $R_L$. As in lab 2, the output voltage must be measured using two probes and finding the differece by the Math function. Measure the DC output voltage and the output ripple voltage, $V_r$. Verify that the ripple frequency of the output is 120Hz (twice the AC input frequency). Also measure the AC input voltage amplitude.\\


Replace the load resistor with a 10 k$\Omega$ resistor and repeat the measurements. Then use the 470 $\Omega$ resistor and repeat once more. A larger resistance draws less load current. \\
\emph{Don't disassemble the circuit as it will be used in part 3!}\\

\textbf{Report}
\begin {enumerate}
\item Present an oscilloscope waveform for at least one load condition, showing the ripple voltage clearly.
\item Create a table or plot showing how DC output voltage and ripple voltage vary with load resistance.
\item Explain why the output voltage decreases and ripple increases with heavier loading (smaller resistance). This unregulated bridge rectifier will serve as the baseline for comparison with the regulated version which will be built later.
\end{enumerate}

\subsection{Part 2: Design of Zener Shunt Regulator}

\emph{Design a Zener shunt regulator to provide a stable output voltage from the bridge rectifier circuit}\\

For this part, reference the shunt regulator circuit from Figure \ref{fig:shunt_reg_concept}. First, determine the characteristics of the Zener diode. (The \hyperlink{https://www.mouser.com/datasheet/3/282/1/1N5221B_1THRU1N5281B_1.pdf}{1N5225 3V zener diode} is provided in the lab) In the datasheet, find the Zener voltage $V_Z$ and the maximum power rating $P_{D}$. \\
%not present: minimum Zener current $I_{Z,\min}$

From Part 1, the the DC output voltage of the bridge rectifier was measured under different load conditions. This will serve as the input voltage $V_{in}$ to the regulator. The regulator will be designed to supply load currents from 0 mA (no load) up to 20 mA (full load) while maintaining regulation. \\
...
\begin{enumerate}
    ...
    \end{equation}
    where $V_{in}$ is the measured DC output from the bridge rectifier (use the value measured with 1 k$\Omega$ load).
    ...
    \end{equation}
    This should be greater than about 10mA, the minimum current required for the Zener to break down.
\end{enumerate}

\textbf{Report}
\begin {enumerate}
\item Show all design calculations in the lab report. If the calculated $R_S$ value does not correspond to an available resistor, select the closest value and recalculate the actual currents and power dissipations.\\
\end{enumerate}


\subsection{Part 3: Implementation and Testing of Shunt Regulator}
\emph{Testing the Zener shunt regulator}\\

Construct the complete regulated power supply by adding the Zener shunt regulator to the bridge rectifier from Part 1. The circuit should include the AC source, bridge rectifier, 47 $\mu$F filter capacitor, series resistor $R_S$ (the value calculated in Part 4), Zener diode, and provision for connecting different load resistors as shown in Figure \ref{fig:regulated_supply}.\\
...
First, test the circuit with no load connected (remove $R_L$). Measure the DC output voltage $V_o$ using the DMM. This should be approximately equal to the Zener voltage $V_Z$. Measure the current through the series resistor by measuring the voltage across it and calculating $I_S = V_{R_S}/R_S$. Since there is no load, this current is entirely flowing through the Zener diode:  $I_Z = I_S$. Verify that the Zener is not overheating—calculate $P_Z = V_Z \cdot I_Z$ and ensure it is within the power rating. \\


Now connect a 10 k$\Omega$ load resistor and measure the output voltage. Calculate the load current as $I_L = V_o/R_L$. The output voltage should remain very close to the no-load value, demonstrating voltage regulation. Measure the voltage across the series resistor to calculate $I_S$, and calculate the Zener current as $I_Z = I_S - I_L$. Replace the load with progressively smaller resistances (between 10k$\Omega$ and 100$\Omega$), measuring the output voltage and calculating the Zener current at each value. \\


Continue decreasing the load resistance until one of the following occurs:  (1) the output voltage begins to drop significantly, indicating loss of regulation, or (2) The minimum safe load resistance is reached, based on component ratings.  Loss of regulation occurs when the Zener current becomes too small to maintain breakdown ($I_Z < I_{Z,\min}$). In all, collect measurements for at least 5 different resistances.\\

\textbf{Report}
\begin {enumerate}
\item \textbf{Please record a video demonstration, or alternately show the lab TA. If recording a video, it should include your face (to verify you recorded it), the circuit functioning, a single measurement, and a breif explaination of what you're doing. Submit this in the \emph{Lab03 Demo} assignment on Canvas.}
\item Construct a table showing the output voltage, load current, series current, Zener current, and Zener power dissipation for each load resistance value tested. Excel will be helpful here.
\item Describe observations about the output voltage as load current increases. At what load current does regulation begin to fail? Explain this in terms of Zener current.
\item Calculate the Zener current for each load condition and verify that regulation is maintained as long as $I_Z \geq I_{Z,\min}$.
\end {enumerate}

\subsection{Part 4: Efficiency Analysis}
\emph{Efficiency Analysis of the Zener shunt regulator}\\

Calculate the efficiency of the regulated power supply under several different load conditions.  For each load resistance value from Part 3 where regulation was maintained, calculate: \\
...
\textbf{Report}
\begin {enumerate}
\item Create a table showing the power dissipations and efficiency for each load condition. Plot efficiency versus load current.
\item Describe qualitatively how efficiency varies with load.  Explain why efficiency is poor at light loads (the Zener dissipates most of the power) and improves at heavier loads (more power goes to the load, less to the Zener).
\item Discuss the fundamental limitation of shunt regulators. They can never achieve high efficiency because they waste power in the series resistor and Zener diode.  This motivates the development of more sophisticated regulators (series regulators and switching regulators).
\end{enumerate}
