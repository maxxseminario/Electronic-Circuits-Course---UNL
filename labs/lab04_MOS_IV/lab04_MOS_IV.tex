...
Use the DMM to measure $V_{DS}$ across the drain-source terminals.

Starting with $V_{GS} = 0$ V, gradually increase $V_{GS}$ from 0 to 5 V in increments of 0.5 V initially. Use finer increments when $V_{DS}$ begins to change rapidly, in order to sufficiently record the transfer characteristic curve. (Obtain $V_{DS}$ values within at least every 1V) Record both voltages in a table.

Calculate the drain current for each voltage using Ohm's law: 
...
\begin{enumerate}
    ...
    \item Vary $V_{DD}$ from 0 V to 5 V in increments of 0.5 V.
    
    \item For each $V_{DD}$ setting, measure $V_{DS}$ and calculate $I_D$. 
...
\begin{enumerate}
    \item Measure $V_{DS}$ and verify saturation mode. $V_{DS} \geq V_{GS} - V_{th}$
    \item Calculate $I_D$.
\end{enumerate}
