...
Using the same circuit as Part 1 (Figure \ref{fig:vth_circuit}), a family of curves will be generated, each corresponding to a different fixed gate-source voltage $V_{GS}$. Use the following three gate-source voltages: $V_{GS} = 2.0$ V, $2.5$ V, and $3.0$ V.

\textbf{Note on the $V_{GS} = 3.0$ V curve:} At this gate voltage, the overdrive voltage $V_{OV} = V_{GS} - V_{th}$ is large, meaning the saturation boundary ($V_{DS} = V_{GS} - V_{th}$) occurs at a higher $V_{DS}$. With a fixed $V_{DD}$ of only 5 V and a 1 k$\Omega$ drain resistor, the voltage drop across $R_D$ at the higher drain currents may prevent $V_{DS}$ from reaching the saturation boundary. It may not be possible to observe a clear saturation region for this curve — record what you can and note this limitation in your report.\\
...
\begin {enumerate}
    ...
    \item Plot all three curves ($V_{GS} = 2.0$ V, $2.5$ V, $3.0$ V) on a single graph with $V_{DS}$ on the x-axis and $I_D$ on the y-axis. Clearly label each curve with its corresponding $V_{GS}$ value. On the plot, identify and label the triode region, saturation region, and the boundary between them. Don't forget the $V_{th}$ boundary line. For the $V_{GS} = 3.0$ V curve, note in the report whether saturation was observable and explain why it may not have been.
...
\textbf{Note on triode region at large $V_{GS}$:} As $V_{GS}$ increases, the drain current rises and the voltage drop across $R_D$ grows, pulling $V_{DS}$ down. At sufficiently large $V_{GS}$, $V_{DS}$ will fall below $V_{GS} - V_{th}$, and the transistor will enter the triode region. Data points where this occurs must be \textbf{excluded} from Plot 2 (which assumes saturation), though they may still be included in Plot 1 to show the full characteristic. You will likely observe this transition for larger $V_{GS}$ values in this sweep.\\

Create a table with columns for $V_{GS}$, $V_{DS}$, $I_D$, and $\sqrt{I_D}$. Note that in this common-source circuit, $V_{DS}$ is also the output voltage of the amplifier ($V_{out} = V_{DS}$) and $V_{GS}$ is the input ($V_{in} = V_{GS}$). \textbf{This dataset will be reused in Part 4 to construct the voltage transfer characteristic.} Using this data, generate the following two plots:\\
...
\textbf{Plot 2: Square-root plot.} Plot $\sqrt{I_D}$ (y-axis) versus $V_{GS}$ (x-axis). Using only the saturation-region points, this should be approximately linear. Perform a linear fit and determine:
\begin{itemize}
    \item The x-intercept, which gives $V_{th}$
    \item The slope, which gives $\sqrt{k_n/2}$, from which $k_n = 2 \times (\text{slope})^2$
\end{itemize}

Compare these values with those obtained in Part 1. They should be consistent.

Examine the quality of the linear fit by calculating the coefficient of determination ($R^2$). A value close to 1 indicates that the square-law model fits the data well. Some deviation from a perfect fit is expected — the following are possible contributing factors for reference; you are not expected to know these in detail:
\begin{itemize}
...
\textbf{Report}
\begin{enumerate}
    \item Provide the table of data used and the two plots with clear labels.
    \item Compare the $V_{th}$ and $k_n$ values obtained here with those from Part 1. Calculate the percentage variation between methods.
    \item Report the $R^2$ value for the Plot 2 linear fit.
    \item Discuss how the gate voltage controls the channel charge density, which in turn controls the drain current. \\
\end{enumerate}


\subsection{Part 4: Voltage Transfer Characteristic}
\emph{In this section, the voltage transfer characteristic (VTC) of the common-source configuration from Part 3 will be analyzed. The VTC illustrates how the circuit transitions between cutoff, saturation, and triode regions as the input voltage is varied. This is a fundamental characteristic that underlies the operation of both amplifiers and digital logic circuits.}\\

\textbf{No new measurements are required.} The circuit used in Part 3 (Figure \ref{fig:vth_circuit}) is equivalent to the common-source amplifier shown in Figure \ref{fig:cs_vtc} — the gate voltage $V_{GS}$ is the input and the drain voltage $V_{DS}$ is the output. Use the Part 3 dataset directly: for each data point, the VTC coordinates are simply $V_{in} = V_{GS}$ and $V_{out} = V_{DS}$, with $V_{DD} = 10$ V and $R_D = 1\ \text{k}\Omega$.\\
...
Using the Part 3 dataset, identify the following three regions in the data:

\begin{enumerate}
    \item \textbf{Cutoff ($V_{in} < V_{th}$):} Transistor is off, $I_D \approx 0$, and $V_{out} \approx V_{DD} = 10$ V.
    \item \textbf{Saturation ($V_{th} < V_{in}$ and $V_{out} \geq V_{in} - V_{th}$):} $V_{out}$ decreases steeply as $V_{in}$ increases — this is the amplification region.
    \item \textbf{Triode ($V_{out} < V_{in} - V_{th}$):} $V_{out}$ is low and relatively insensitive to further increases in $V_{in}$.
\end{enumerate}

The saturation-to-triode transition occurs when $V_{out} = V_{in} - V_{th}$. Identify this point in the dataset.

Calculate the incremental voltage gain at several points in the saturation region:

\begin{equation}
    \left|A_v\right| = \left|\frac{\Delta V_{out}}{\Delta V_{in}}\right|
\end{equation}

Use closely spaced data points (e.g., at $V_{in} = V_{th} + 0.5$ V, $V_{th} + 1.0$ V, etc.) and note how the gain varies with bias point.

\textbf{Report}
\begin{enumerate}
    \item \textbf{Please record a video demonstration, or alternately show the lab TA. If recording a video, it should include your face, the circuit functioning, a single measurement, and a brief explanation of what you're doing. Submit this in the \emph{Lab04 Demo} assignment on Canvas.}
    \item Plot of the VTC with $V_{in}$ on the x-axis and $V_{out}$ on the y-axis. Label the three regions and mark the transition points and incremental gain values.
    \item Discuss the significance of the VTC. Consider for analog amplifiers, the saturation region provides gain.
\end{enumerate}
