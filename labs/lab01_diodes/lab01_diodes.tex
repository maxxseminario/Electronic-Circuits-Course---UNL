...
\emph{For each part, please follow the instructions to preform the experiment. For the report, provide the requested data and answer any questions.}
...
Start with the power supply set to 0 V, and gradually increase it to 1V in small incraments of around 0.1 V.  For each incrament, record the voltage across the diode, $V_D$, and the current through the diode, $I_D$. Keep increasing the supply voltage up to 6V, using larger incraments of around .5 to 1 V. Very little current will flow until the diode voltage reaches approximately 0.5 to 0.6 V, after this point the current increases exponentially. Exercise caution not to exceed the current rating of the diode, as it will permanently damage the device.  If the diode is heating up noticeably, reduce the current immediately.\\
...
\begin {enumerate}
    \item Plot the I-V characteristic (Voltage v. Current) of the diode with clearly labeled axes.
    \item Describe the shape of the curve and how it relates to the Shockley diode equation.
    \item Explain why very little current flows below a certain voltage threshold, but a large current flows once it is reached.
\end {enumerate}
...
where $\Delta V_D$ and $\Delta I_D$ represent small changes in voltage and current around the operating point. To obtain accurate results, take a value on either side of the operating point, and calculate the slope between these two values. A smaller interval used (while still maintaining measurement accuracy) will produce a better approximation of the true small signal resistance. \\

After calculating the experimental small signal resistance values, compare them with the theoretical predictions from Equation \ref{eq:dynamic_r}. Use an appropriate ideality factor, $n=1$, and use the measured DC current at each operating point. Calculate the percentage error between measured and theoretical values.\\

\textbf{Report}
\begin {enumerate}
    \item Provide the experimental measurements and $r_d$ calculation.
    \item Provide the theoretical $r_d$ calculation and comparison to experimental values.
    \item Dicuss observations about small signal resistance relationship to operating current. Explain physically why the a small signal resistance decreases as the forward current increases.
    \item Discuss possible sources of error in the measurements, and how the accuracy of the small signal resistance calculations are affected. \\
\end {enumerate}

\subsection{Part 3: Reverse-Bias Characteristics of Silicon Diodes}
\emph{Behavior of a silicon diode under reverse-bias conditions}\\

Carefully remove the diode from the circuit and reverse its orientation so that the cathode is now connected toward the positive supply terminal and the anode toward ground as shown in Figure \ref{fig:reverse_circuit}.  This reverse-biases the diode.  To limit the current in case of accidental breakdown, change the resistor value to 470 $\Omega$. \\
...
Starting from 0 V, gradually increase the supply voltage, which now applies an increasing reverse voltage across the diode.  Take measurements of the reverse voltage $V_D$ and the reverse current $I_D$ at increments of 1V, up to 10V.

The reverse current should be extremely small, likely in the range of nanoamperes or microamperes, and may be difficult to measure accurately with standard multimeters. The current should remain relatively constant the reverse voltage is increased, consistent with the saturation current $I_S$ predicted by the Shockley equation.\\

\textbf{Report}
\begin {enumerate}
    \item Provide reverse-bias measurements and describe any observations.
    \item Explain why the reverse current is so much smaller than the forward current at equivalent voltage magnitudes. Discuss the physical mechanism responsible for this reverse current (minority carrier diffusion).
    \item If reverse current could not be measured accurately due to instrument limitations, explain this limitation and estimate an upper bound on the reverse current based on the measurements. \\
\end {enumerate}


\subsection{Part 4: Zener Diode Breakdown Characteristics}
\emph{Voltage regulation properties of a Zener diode operating in the breakdown region}\\

Construct the circuit shown in Figure \ref{fig:zener_circuit} using a Zener diode and a 100 $\Omega$ resistor. Pay careful attention to the polarity:  for the Zener to operate in breakdown, its cathode (the marked end) must be connected toward the positive terminal of the supply through the resistor, and its anode must be connected to ground. This is the reverse of normal diode operation. \\
...
When the applied reverse voltage is below the Zener voltage, very little current flows and the voltage across the Zener approximately equals the supply voltage minus the resistor drop. Once the supply voltage exceeds the Zener voltage, the diode enters breakdown.  In breakdown, the voltage across the Zener should remain relatively constant (near its rated $V_Z$ value) even as the current increases significantly. This is the voltage regulation behavior that makes Zener diodes useful. The exact value of the Zener voltage depends on the particular diode being used.\\

Continue increasing the supply voltage up to 10 V, taking measurements at each step. Constantly monitor the power dissipation in the Zener diode, calculated as $P_Z = V_Z \times I_Z$.  Do not exceed the power rating of the Zener diode. If it becomes noticeably warm to the touch, the power limit is likely being reached and current should not be increased any further. \\

\textbf{Report}
\begin {enumerate}
    \item Plot the I-V characteristic of the Zener diode showing both the low-current region (before breakdown) and the breakdown region. Clearly identify the Zener breakdown voltage.
    \item Describe observations about the voltage regulation behavior. How much does the Zener voltage change as the current varies over a wide range?
    \item Explain the physical mechanism responsible for Zener breakdown. Discuss why this constant-voltage property is useful in practical circuits. \\
\end {enumerate}


\subsection{Part 5: Zener Diode Small Signal Resistance}
\emph{Using data from Part 4, the small signal resistance of the Zener diode in breakdown region is found}\\

Referncing the plot from part 4, select two measurement points where the Zener is clearly in breakdown (i.e., where the supply voltage significantly exceeds the Zener voltage). Choose points with substantially different currents in order to get a meaningful calculation. For example, one point where $V_S \approx 7$ V and another where $V_S \approx 10$ V.\\
...
\textbf{Report}
\begin {enumerate}
    \item Calculate the Zener small signal resistance and discuss what this value indicates about the quality of voltage regulation provided by the Zener diode.
    \item If available, compare the measured $r_z$ with the datasheet specification for the particular Zener diode used.
    \item Explain how the small signal resistance affects the performance of a Zener diode in a voltage regulator circuit. Why is a smaller small signal resistance desirable? \\
\end {enumerate}
...
\begin{enumerate}
    ...
    \item A diode is conducting a forward current of 10 mA at room temperature. Calculate its theoretical small-signal resistance assuming $n = 1$. Show all work.
    ...
    \item Research and briefly explain how temperature affects the forward voltage drop of a silicon diode. What is the typical temperature coefficient (in mV/°C)? How might this affect the measurements if the diode heats up during testing?
\end{enumerate}
