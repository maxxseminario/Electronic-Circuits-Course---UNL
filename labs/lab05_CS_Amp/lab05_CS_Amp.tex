...
\end{circuitikz}
\caption{Simplified common-source amplifier with DC-offset gate supply and capacitive load.}
\label{fig:cs_basic}
\end{figure}
...
With a load resistor $R_L$ connected through the output coupling capacitor $C_{L}$, the effective load becomes $R_D \parallel R_L$, and the voltage gain to the output becomes:
...
\textit{In this part, you will measure the AC voltage gain of the CS amplifier by superimposing a small AC signal on the DC gate bias. The simplified circuit allows you to directly observe amplification without the complexity of coupling capacitors.} \\

Now you will measure the AC voltage gain of the common-source amplifier. Add the output coupling capacitor $C_{L}$ to your circuit (between the drain and load resistor).  Choose a large capacitor (e.g., 10 $\mu$F to 100 $\mu$F) to ensure it acts as a short circuit at the signal frequencies you'll use (e.g., 1 kHz). \\
...
Initially, do not connect a load resistor ($C_L = \infty$, or equivalently, open circuit at the output).\\

Measure the AC voltages using an oscilloscope: 

\begin{itemize}
    \item $v_{in}$: Measure the AC component at the gate. This is your input signal.
    \item $v_{out}$: Measure the AC component at the output (after $C_L$).
\end{itemize}
...
\begin{circuitikz}[american]
  ...
  % Load resistor to the right of capacitor
  \draw (8.5,3.8) to[R, l=$R_{L}$] (8.5,0);
  
  % Connect resistor to output node
  \draw (VOUT) -- (8.5,3.8);
  
  % Output terminal
  \draw (8.5,3.8) to[short, -o] (9.5,3.8) node[right] {$v_{out}$};
  
  % Voltage label
  \node[above] at (0,3) {$V_G + v_{in}$};
  
\end{circuitikz}
\caption{Common-source amplifier with resistive and capacitive load for measuring loading effect.}
\label{fig:cs_loaded}
\end{figure}
...
With the circuit from Part 2, connect various load resistors $R_L$ from the output to ground (after the coupling capacitor $C_L$). Use the following values:  10 k$\Omega$, 2.2 k$\Omega$, and 470 $\Omega$.
