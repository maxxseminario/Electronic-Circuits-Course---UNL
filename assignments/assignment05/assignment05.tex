...
\begin{itemize}
    ...
    \item Attach your \textbf{MATLAB code as a separate document or as appendix}
    \item Include labeled plots with time-aligned signal segments
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Hold Interpolation:} Implement as a causal FIR filter - you may use the \texttt{filter()} function

    \item \textbf{Linear Interpolation:} Implement as a causal FIR filter using - you may use the \texttt{filter()} function

    \item \textbf{Short FIR Lowpass Filter:} Design a shorter FIR interpolation filter ($\sim$20--200 taps) - you may use \texttt{fir1()}

    \item \textbf{Long FIR Lowpass Filter:} Design a longer FIR interpolation filter ($\sim$200--2000 taps) - you may use \texttt{fir1()}
\end{enumerate}
